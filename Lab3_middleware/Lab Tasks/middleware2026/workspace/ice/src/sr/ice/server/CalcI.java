...
public class CalcI implements Calc {
	...
	private String currentIdentity(Current current) {
		return current.id.category + "/" + current.id.name;
	}

	@Override
	public long add(int a, int b, Current __current) {
		System.out.println("ADD [" + currentIdentity(__current) + "]: a = " + a + ", b = " + b + ", result = " + (a + b));
	...
	public long subtract(int a, int b, Current __current) {
		System.out.println("SUBTRACT [" + currentIdentity(__current) + "]: a = " + a + ", b = " + b + ", result = " + (a - b));
		return a - b;
	}
	...
	public /*synchronized*/ void op(A a1, short b1, Current current) {
		System.out.println("OP" + (++counter) + " [" + currentIdentity(current) + "]");
		try {
			Thread.sleep(500);
